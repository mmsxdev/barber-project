...
export default function Commissions() {
  ...
  const handleBarberChange = (e) => {
    ...
    setSearchParams({ mode: isEditMode ? 'edit' : '', barberId: newBarberId || '' }, { replace: true });
  };

  const navigateToEdit = () => {
    setSearchParams({ mode: 'edit', barberId: selectedBarber }, { replace: true });
  };
  ...
  return (
    <div className={`p-4 sm:p-6 lg:p-8 min-h-screen space-y-6 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
        ...
        <div className="flex items-center gap-3">
            ...
            <div>
                ...
                <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    {isEditMode ? 'Defina as taxas de comissão' : 'Visualize as comissões dos barbeiros'}
                </p>
            </div>
        </div>
      ...
      {error && (
        <div className={`p-4 border rounded-lg shadow-sm text-sm ${isDarkMode ? 'bg-red-900/20 border-red-700/30 text-red-300' : 'bg-red-50 border-red-200 text-red-700'}`}>
          {error}
        </div>
      )}

      <div className={`p-4 sm:p-6 rounded-xl border shadow-lg ${isDarkMode ? 'bg-gray-800/80 border-gray-700' : 'bg-white border-gray-200'}`}>
        <label htmlFor="barber" className={`block text-sm font-medium mb-1.5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
        ...
        <select
          ...
          className={`block w-full rounded-lg border p-1 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 disabled:opacity-60 disabled:cursor-not-allowed ${isDarkMode ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-400' : 'border-gray-300 bg-white text-gray-900 placeholder-gray-400'}`}
          disabled={loading || barbers.length === 0}
        >
          <option value="">{loading ? "Carregando..." : (barbers.length === 0 ? "Nenhum barbeiro" : "Selecione um barbeiro")}</option>
          {barbers.map(barber => (
      ...
      <div className="mt-6">
        {selectedBarber ? (
          isEditMode ? (
            <EditCommissions barberId={selectedBarber} onBack={navigateToReport} />
          ) : (
            <CommissionReport barberId={selectedBarber} />
          )
        ) : (
          !loading && barbers.length > 0 && (
            <div className={`text-center p-6 rounded-xl border shadow-lg ${isDarkMode ? 'bg-gray-800/80 border-gray-700 text-gray-400' : 'bg-white border-gray-200 text-gray-500'}`}>
              Selecione um barbeiro para visualizar o relatório ou editar as comissões.
            </div>
          )
        )}
      </div>
